"""
Qubit Scaling — N=8, noiseless only (standalone, run independently)
Usage: python run_qubit_8_noiseless.py
Appends one row (noise_p=0.0) to results/scaling_qubit.csv
"""
...
def main():
    log(f"=== N=8 noiseless scaling (subsampled {N_TRAIN}/{N_TEST}) ===")
    ...
    Xq_tr = extract_all(tr_raw, extractor, "N=8 Train")
    Xq_te = extract_all(te_raw, extractor, "N=8 Test")
    ...
    log(f"RESULT: N=8 p=0 RMSE={rmse:.4f} time={elapsed:.1f}s alpha={m.alpha_}")
